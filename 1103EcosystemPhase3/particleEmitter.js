...
ParticleEmitter.prototype.generateNewParticles = function(targetDir,ctx){
  ...
  for(var i = 0;i<n;i++){
    ...
    if(this.particleType == particleTypes.CIRCLE){
      newParticle = new CircleParticle(new JSVector(this.pos.x,this.pos.y),vel,size/2,this.lifeTime,this.scaleR,this.scaleG,this.scaleB,this.isMonochrome,ctx);
    }
    else if(this.particleType == particleTypes.SQUARE){
      newParticle = new SquareParticle(new JSVector(this.pos.x,this.pos.y),vel,size,this.lifeTime,this.scaleR,this.scaleG,this.scaleB,this.isMonochrome,ctx);
    }
    else if(this.particleType == particleTypes.TRIANGLE){
      newParticle = new TriangleParticle(new JSVector(this.pos.x,this.pos.y),vel,size,this.lifeTime,this.scaleR,this.scaleG,this.scaleB,this.isMonochrome,ctx);
    }
