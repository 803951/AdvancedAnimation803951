...
var cnv,ctx,species,matingProb;
...
function init(){
  ...
  for(var i = 0;i<mice;i++){
    ...
    let mouse = new Mouse(pos,radius,1,0,matingProb);
    species.push(mouse);
  }
  ...
  for (var i = 0;i<species.length;i++){
    ...
    for(var k = 0;k<species.length;k++){
      if(species[i].pos.distance(species[k].pos)<=species[i].radius+species[k].radius){
        if(species[i].sex!=species[k].sex&&species[i].inHeat&&species[k].inHeat){
          species[i].inHeat = false;
          species[k].inHeat = false;
          let pos = JSVector.addGetNew(species[i].pos,species[k].pos);
          pos.divide(2);
          let newMouse = new Mouse(pos,Math.random()*5+17.5,1,0,matingProb);
